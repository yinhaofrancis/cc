...
void cc::EventQueue::change(struct kevent &e) const
{
    kevent(m_kq, &e, 1, nullptr, 0, 0);
}
void cc::EventQueue::add(uintptr_t ident, int16_t filter, uint32_t fflag, intptr_t data) const
{
    change(ident, filter, EV_ADD, fflag, data);
}
void cc::EventQueue::addOnce(uintptr_t ident, int16_t filter, uint32_t fflag, intptr_t data) const
{
    change(ident, filter, EV_ADD | EV_ONESHOT, fflag, data);
}
void cc::EventQueue::addClear(uintptr_t ident, int16_t filter, uint32_t fflag, intptr_t data) const
{
    change(ident, filter, EV_ADD | EV_CLEAR, fflag, data);
}
void cc::EventQueue::remove(uintptr_t ident, int16_t filter) const
{
    change(ident, filter, EV_DELETE, 0, 0);
}
void cc::EventQueue::enable(uintptr_t ident, int16_t filter, uint32_t fflag, intptr_t data) const
{
    change(ident, filter, EV_ENABLE, fflag, data);
}
void cc::EventQueue::disable(uintptr_t ident, int16_t filter) const
{
    change(ident, filter, EV_DISABLE, 0, 0);
}
void cc::EventQueue::change(uintptr_t ident, int16_t filter, uint16_t flag, uint32_t fflag, intptr_t data) const
{
...
void cc::EventQueue::add_timer(uint timerId, TimerConfig config, long data) const
{
    change(timerId,EVFILT_TIMER,EV_ADD,config,data);
}

void cc::EventQueue::add_signal(uint signal, intptr_t data) const
{
    change(signal,EVFILT_SIGNAL,EV_ADD,0,data);
}

void cc::EventQueue::add_fd_writable(int fd) const
{
    change(fd,EVFILT_WRITE,EV_ADD | EV_EOF,0,0);
}

void cc::EventQueue::add_fd_readable(int fd,ReadableConfig config,intptr_t data) const
{
...
void cc::EventQueue::add_process(pid_t pid,ProcessConfig config) const{
    add(pid,EVFILT_PROC,config,0);
}

void cc::EventQueue::add_vnode(int fd,VnodeConfig config) const{
